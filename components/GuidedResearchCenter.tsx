...
async function logout() { await fetch("/api/auth/sign-out", { method: "POST", headers: { "Content-Type": "application/json", Origin: location.origin }, body: "{}" }); location.href = "/login"; }
  ...
  function sendCandidateToNavigator(candidate: DomainTopicLabCandidate) {
    // V3-U03-FULL §6: adopt topic -> SAME Project -> navigation context (no new Project, no re-entry)
    if (currentProject) {
      setNavigatorTopic(candidateToNavigatorTopic(candidate));
      setActiveNav("navigator");
      return;
    }
    // No project yet: reuse existing adopt-to-S0 flow (user confirms -> creates the one Project)
    setNavigatorTopic(candidateToNavigatorTopic(candidate));
    adoptTopicLabDraft(candidate);
    setPendingNavigatorAfterCreate(true);
  }
