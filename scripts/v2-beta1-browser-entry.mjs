...
try {
  if (!path.isAbsolute(verifierPath ?? "") || path.basename(verifierPath) !== "verify-v2-beta1-browser.mjs") throw new Error("BETA1_BROWSER_ENTRY_ARGUMENTS_INVALID");
  ...
  console.log(JSON.stringify({ status: "BLOCKED", activeStage: "ENTRY_IMPORT_OR_BROWSER_LAUNCH", reasonClass: error?.name === "TimeoutError" ? "TIMEOUT" : error?.name === "AssertionError" ? "ASSERTION" : "BROWSER_RUNTIME", subgate: error?.message === "BETA1_BROWSER_ENTRY_ARGUMENTS_INVALID" ? "ARGUMENTS" : "IMPORT_OR_LAUNCH" }));
  process.exitCode = 1;
}
