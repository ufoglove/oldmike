...
$logs = Join-Path ([IO.Path]::GetFullPath([IO.Path]::GetTempPath())) ("oldmike-v2-alpha4-browser-" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $env:V2_ALPHA4_BASE_URL = "http://127.0.0.1:$webPort"
  $env:BETTER_AUTH_URL = $env:V2_ALPHA4_BASE_URL
  ...
  for ($attempt=0; $attempt -lt 160; $attempt++) {
    ...
    try { $response=Invoke-WebRequest -UseBasicParsing -Uri ("http://127.0.0.1:{0}/v2-alpha4-local" -f $webPort) -TimeoutSec 1; if ($response.StatusCode -eq 200) { $ready=$true; break } } catch {}
    Start-Sleep -Milliseconds 250
  }
  if (-not $ready) { throw "NEXT_SERVER_TIMEOUT" }
  node scripts\verify-v2-alpha4-browser.mjs
  if ($LASTEXITCODE -ne 0) { throw "BROWSER_TEST_FAILED" }
  $exitCode = 0
} catch {
  Write-Output "V2_ALPHA4_BROWSER=FAIL"
  Write-Output ("FIRST_FAILED_STAGE=" + $_.Exception.Message)
} finally {
  if ($server) { Stop-ExactProcessTree $server.Id }
  foreach ($name in @("TEST_FIXTURE","OLD_MIKE_V2_ALPHA2_SYNTHETIC_PRINCIPAL","OLD_MIKE_V2_ALPHA4_LOCAL_PROTOTYPE","NEXT_TELEMETRY_DISABLED","V2_ALPHA4_BASE_URL","BETTER_AUTH_URL")) { Remove-Item ("Env:" + $name) -ErrorAction SilentlyContinue }
  $cleaned = $false
  ...
  if (-not $cleaned -and (Test-Path -LiteralPath $logs)) { Write-Output "V2_ALPHA4_BROWSER_CLEANUP=FAIL_TEMP_RETAINED"; $exitCode = 2 }
  else { Write-Output "V2_ALPHA4_BROWSER_CLEANUP=PASS_ZERO_TEMP_PROCESS" }
}
exit $exitCode
