...
$cluster = Join-Path $tempBase ("oldmike-v1528-m03-" + [Guid]::NewGuid().ToString("N"))
$logs = Join-Path $tempBase ("oldmike-v1528-m03-logs-" + [Guid]::NewGuid().ToString("N"))
...
$started = $false
try {
  & (Join-Path $pgBin "initdb.exe") -D $cluster -A trust -U postgres --no-locale *> (Join-Path $logs "initdb.log")
  ...
  $started = $true
  & (Join-Path $pgBin "createdb.exe") -h 127.0.0.1 -p $port -U postgres oldmike_m03_review *> (Join-Path $logs "createdb.log")
  $env:INTEGRATION_DATABASE_URL = "postgresql://postgres@127.0.0.1:$port/oldmike_m03_review"
  $env:DATABASE_URL = $env:INTEGRATION_DATABASE_URL
  $env:INTEGRATION_DATABASE_DISPOSABLE = "1"; $env:INTEGRATION_TEST_MODE = "1"; $env:TEST_FIXTURE = "1"
  $env:BETTER_AUTH_URL = "http://127.0.0.1:$port"; $env:BETTER_AUTH_SECRET = "disposable-m03-secret-not-for-production"; $env:REGISTRATION_MODE = "closed"; $env:ACCOUNT_PROVISIONING_MODE = "admin_only"
  node --experimental-strip-types --experimental-loader ./scripts/disposable-server-only-loader.mjs scripts/verify-review-studio-disposable.mjs
  if ($LASTEXITCODE -ne 0) { throw "Review Studio disposable verifier failed" }
} finally {
