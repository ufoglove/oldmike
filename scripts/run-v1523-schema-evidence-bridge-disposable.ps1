...
$cluster = Join-Path $tempBase ("oldmike-v1523-schema-evidence-" + [Guid]::NewGuid().ToString("N"))
$logs = Join-Path $tempBase ("oldmike-v1523-schema-evidence-logs-" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $server = Start-Process -FilePath (Join-Path $pgBin "pg_ctl.exe") -ArgumentList @(
    ...
    "-o", "`"-h 127.0.0.1 -p $port -c allow_system_table_mods=on`"", "-w", "start"
  ) -WindowStyle Hidden -PassThru
  if (-not $server.WaitForExit(30000) -or $server.ExitCode -ne 0) { throw "PG_START_FAILED" }
  $started = $true
  & (Join-Path $pgBin "createdb.exe") -h 127.0.0.1 -p $port -U postgres oldmike_schema_evidence *> (Join-Path $logs "createdb.log")
  if ($LASTEXITCODE -ne 0) { throw "CREATE_DATABASE_FAILED" }

  $env:INTEGRATION_DATABASE_URL = "postgresql://postgres@127.0.0.1:$port/oldmike_schema_evidence"
  $env:DATABASE_URL = $env:INTEGRATION_DATABASE_URL
  $env:INTEGRATION_DATABASE_DISPOSABLE = "1"
  node scripts/verify-schema-evidence-bridge-disposable.mjs
  $code = $LASTEXITCODE
  if ($code -ne 0) { throw "SCHEMA_EVIDENCE_DISPOSABLE_FAILED" }
} finally {
  foreach ($name in @("INTEGRATION_DATABASE_URL","DATABASE_URL","INTEGRATION_DATABASE_DISPOSABLE")) {
