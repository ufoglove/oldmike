...
$cluster = Join-Path $tempBase ("oldmike-foundation-runtime-" + [Guid]::NewGuid().ToString("N"))
$logs = Join-Path $tempBase ("oldmike-foundation-runtime-logs-" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $started = $true
  & (Join-Path $pgBin "createdb.exe") -h 127.0.0.1 -p $port -U postgres oldmike_foundation_runtime *> (Join-Path $logs "createdb.log")
  if ($LASTEXITCODE -ne 0) { throw "CREATE_DATABASE_FAILED" }

  $env:INTEGRATION_DATABASE_URL = "postgresql://postgres@127.0.0.1:$port/oldmike_foundation_runtime"
  ...
  $env:BETTER_AUTH_SECRET = "disposable-foundation-runtime-secret-not-for-production"
  $env:REGISTRATION_MODE = "closed"
  $env:ACCOUNT_PROVISIONING_MODE = "admin_only"
  node --experimental-strip-types --experimental-loader ./scripts/disposable-server-only-loader.mjs scripts/verify-foundation-runtime-disposable.mjs
  $code = $LASTEXITCODE
  if ($code -ne 0) { throw "FOUNDATION_RUNTIME_DISPOSABLE_FAILED" }
} finally {
  ...
  Write-Output "DISPOSABLE_DATABASE_PROCESSES=0"
  Write-Output "DISPOSABLE_DATABASE_TEMP=0"
  Write-Output "DATABASE_WRITES_ONLINE=0"
}
exit $code
