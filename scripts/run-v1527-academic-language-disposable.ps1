...
$cluster = Join-Path $tempBase ("oldmike-v1527-m02-" + [Guid]::NewGuid().ToString("N"))
$logs = Join-Path $tempBase ("oldmike-v1527-m02-logs-" + [Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $started = $true
  & (Join-Path $pgBin "createdb.exe") -h 127.0.0.1 -p $port -U postgres oldmike_m02 *> (Join-Path $logs "createdb.log")
  if ($LASTEXITCODE -ne 0) { throw "CREATE_DATABASE_FAILED" }

  $env:INTEGRATION_DATABASE_URL = "postgresql://postgres@127.0.0.1:$port/oldmike_m02"
  ...
  $env:BETTER_AUTH_SECRET = "disposable-m02-secret-not-for-production"
  $env:REGISTRATION_MODE = "closed"
  $env:ACCOUNT_PROVISIONING_MODE = "admin_only"
  node --experimental-strip-types --experimental-loader ./scripts/disposable-server-only-loader.mjs scripts/verify-academic-language-disposable.mjs
  $code = $LASTEXITCODE
  if ($code -ne 0) { throw "M02_DISPOSABLE_CONTRACT_FAILED" }
} finally {
