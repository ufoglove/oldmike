...
try {
  if (!path.isAbsolute(verifierPath ?? "") || path.basename(verifierPath) !== "verify-v2-alpha6-r1-browser.mjs") throw new Error("ALPHA6_R1_BROWSER_ENTRY_ARGUMENTS_INVALID");
  ...
  console.log(JSON.stringify({ status: "BLOCKED", activeStage: "ENTRY_IMPORT_OR_BROWSER_LAUNCH", reasonClass: error?.name === "TimeoutError" ? "TIMEOUT" : error?.name === "AssertionError" ? "ASSERTION" : "BROWSER_RUNTIME" }));
  process.exitCode = 1;
}
