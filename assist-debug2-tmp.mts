...
const request = {
  ...
  contextSnapshot: { current: "生成式 AI 輔助個人化學習對大學生自主學習動機與學習成效的影響", fieldStatus: {}, selected: "", schema: { type: "TEXT", maxLength: 200 } },
};
const contextHash = sha256Canonical({ scope: "PRE_PROJECT", surface: "S0_INTAKE", targetKind: "FIELD", schemaId: "researchDirection", sourceHash: request.sourceHash });
const started = Date.now();
const result = await executeOldMikeAssist({
  request: request as never,
  context: { scope: "PRE_PROJECT", formalProjectContext: false, snapshot: request.contextSnapshot },
  contextHash,
...
if (result.kind === "success") {
  ...
  result.value.issues.forEach((i) => console.log("  issue:", i.code, JSON.stringify(i.fields), i.message.slice(0, 60)));
  result.value.outputs.slice(0, 2).forEach((o) => console.log("  output:", JSON.stringify(o).slice(0, 150)));
