...
export function consensusCapabilitySnapshot(): ProviderCapabilitySnapshot {
  return {
    provider: "CONSENSUS",
    snapshotVersion: "consensus/1.0.0",
    checkedAt: FEDERATED_CAPABILITY_CHECKED_AT,
    billingPoolId: "consensus_account_pool",
    capabilities: {
      search: cap("documented", "quick search API returns top 20 results"),
      metadata: cap("documented", "citation count, publish date per paper"),
      references: cap("unknown"),
      citations: cap("unknown"),
      abstract: cap("unknown", "metadata/abstract level depends on account tier; not full text by default"),
      ...
      study_type_filter: cap("documented", "advanced filters incl. study designs per official pages"),
