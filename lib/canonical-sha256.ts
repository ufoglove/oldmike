...
function canonicalize(value: unknown): unknown {
  ...
  if (typeof value === "number") return Number.isFinite(value) ? (Object.is(value, -0) ? 0 : Number(value.toString())) : null;
...
function rotateRight(value: number, count: number) { return (value >>> count) | (value << (32 - count)); }
...
export function sha256CanonicalPortable(value: unknown) { return sha256Utf8(JSON.stringify(canonicalize(value))); }
