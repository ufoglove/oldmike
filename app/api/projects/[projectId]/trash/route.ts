...
export async function POST(_request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    const result = await tenantProjectRepository.trash(identity, projectId);
    if (!result.trashed) return NextResponse.json({ ok: false, code: "project_not_found", error: "找不到可移至回收筒的專案（僅限自己的專案，且未在回收筒）。" }, { status: 404 });
    return NextResponse.json({ ok: true, trashed: true, projectId }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    ...
    return NextResponse.json({ ok: false, code: "project_trash_failed", error: "移至回收筒失敗。" }, { status: 500 });
  }
}
