...
function canonicalValue(value: unknown): string {
  ...
  if (typeof value === "object" && value !== null) return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${canonicalValue((value as Record<string, unknown>)[key])}`).join(",")}}`;
...
const contextHash = sha256Canonical({ scope: "PRE_PROJECT", surface: "S0_RESEARCH_TEXT", targetKind: "FIELD", schemaId: "researchDirection", sourceHash });
...
if (result.kind === "success") {
  console.log("SUCCESS — issues:", result.value.issues.length, "| suggestions:", result.value.suggestions.length, "| canApply:", result.value.canApply, "| completion:", result.value.completionClass);
  result.value.issues.forEach((i) => console.log("  issue:", i.code, JSON.stringify(i.fields), i.message.slice(0, 80)));
  result.value.suggestions.slice(0, 2).forEach((s) => console.log("  suggestion:", JSON.stringify(s).slice(0, 200)));
